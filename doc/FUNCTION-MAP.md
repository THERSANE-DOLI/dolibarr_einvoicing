...
*module 1.2.0, commit e8a41abb, generated 2026-09-17. Clickable version: [function-map.html](function-map.html).*
...
| `_findOrCreateProductFromEinvoiceLine()` | `class/protocols/CommonProtocol.class.php:1274` | Creates the product when the setup allows it, guessing its type from what the line says. Otherwise the line falls back on the vendor's default routing product, or stays a free line. | _detectProductTypeFromEinvoiceLine(), the 'product' routing of the vendor | Product::create() |
...
| `Document::cleanXmlData()` | `class/document.class.php:1629` | Strips the attachments out of the received XML before it is stored, and checkXmlDataMaxSize() keeps it under the MEDIUMTEXT ceiling. | the raw XML | einvoicing_document.xml_data |
...
| `EINVOICING_KEEP_RECEIVED_XML_AS_IS` | IN | hidden: keep the attached XML file byte for byte as the access point returned it; the embedded files are stored as attached files either way, and the copy in database stays scrubbed |
